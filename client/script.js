...
const changeHelloBtnColor = (event) => {

    document.getElementById('say-hello-button').style.background= `#000000`;
    document.getElementById(`say-hello-button`).style.color=`#FFFFFF`;
    
}


sayHelloButton.addEventListener(`mouseover`, changeHelloBtnColor)
...
const changeBack = (event) => {
    ...
    document.getElementById(`say-hello-button`).style.color=`#000000`;
}
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/I-NEED-AN-ADULT`)
    .then((res) => {
...
let repeatBt = document.getElementById(`repeat-button`)

repeatBt.addEventListener(`click`, repeatMyParam)
...
const queryTest = () => {
    axios.get(`http://localhost:3000/query-test?help&saveme`)
    .then(res => {
        console.log(res.data)
    })
}
...
const createFood = (event) => {
    ...
    const foodSection = document.querySelector(`#food-input`)
    ...
    axios.post(`http://localhost:3000/food`, bodyObj)
        .then(res => {
         console.log(res.data)
         for(let i = 0; i < res.data.length; i++){
            const foodPara = document.createElement(`p`)
            foodPara.textContent= res.data[i]
            fooodSection.appendChild(foodPara)
         }
     })
